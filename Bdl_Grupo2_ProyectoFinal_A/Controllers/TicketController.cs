...
namespace Bdl_Grupo2_ProyectoFinal_A.Controllers
{
    ...
    {
        private Bdl_ProyectoFinal_AEntities db = new Bdl_ProyectoFinal_AEntities();
        ...
        public ActionResult View_Ticket(int? id)
        {
            if (Session["Tec_Id"] != null)
            {
                ...
                if (oReader1.HasRows && oReader1.Read())
                {
                    ...
                    n.Tkt_Fecha_Hora = oReader1["DATE"].ToString();
        ...
        [HttpPost]
        public ActionResult ReasignarAleatorio(string id, string Tec_Id)
        {
            ...
            if (Convert.ToInt32(Session["Prf_Id"]) == 2)
            {
                if (Session["Tec_Id"] != null)
                {
                    ...
                    try
                    {
                        string query = "UPDATE Tickets SET Tec_Id = (SELECT TOP 1 T.Tec_Id FROM Tecnicos AS T " +
                                       "INNER JOIN [dbo].[UsrTecnicos] AS U ON U.Tec_Id = T.Tec_Id " +
                                       "WHERE U.Tec_Id = T.Tec_Id AND T.Prf_Id = 1 AND T.Tec_Id != " + Tec_Id + "ORDER BY NEWID()) WHERE Tkt_Id = " + id;
                        con.ConsultaBd(query);
                        return Redirect("/Ticket/View_Ticket/" + id);
                    }
                    catch
                    {
                        return RedirectToAction("ListTickets", "Ticket");
        ...
        [HttpPost]
        public ActionResult Reasignar(string id, string DPI)
        {
            ...
            if (Convert.ToInt32(Session["Prf_Id"]) == 2)
            {
                if (Session["Tec_Id"] != null)
                {
                    ...
                    try
                    {
                        string query = "UPDATE Tickets SET Tec_Id = (SELECT Tec_Id FROM Tecnicos WHERE DPI = '" + DPI + "') WHERE Tkt_Id = " + id;
                        con.ConsultaBd(query);
                        return Redirect("/Ticket/View_Ticket/" + id);
                    }
                    catch
